...
type Menu struct {
	Id         int     `orm:"column(id_menu);pk"`
	IdPagina   float64 `orm:"column(id_pagina)"`
	NombreMenu string  `orm:"column(nombre_menu)"`
	Tooltip    string  `orm:"column(tooltip)"`
	Rolmenu    float64 `orm:"column(rolmenu)"`
	Orden      float64 `orm:"column(orden)"`
	Parametros string  `orm:"column(parametros);null"`
}

...
func init() {
	orm.RegisterModel(new(Inhabilidad))
}

// AddInhabilidad insert a new Inhabilidad into database and returns
// last inserted Id on success.
func AddInhabilidad(m *Inhabilidad) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetInhabilidadById retrieves Inhabilidad by Id. Returns error if
// Id doesn't exist
...
// GetAllInhabilidad retrieves all Inhabilidad matches certain condition. Returns empty list if
// no records exist
func GetAllInhabilidad(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Inhabilidad))
	// query k=v
	for k, v := range query {
...
// UpdateInhabilidad updates Inhabilidad by Id and returns error if
...
// DeleteInhabilidad deletes Inhabilidad by Id and returns error if

...
type OrdenadorGasto struct {
	Id              int     `orm:"column(id_ordenador);pk"`
	NombreOrdenador string  `orm:"column(nombre_ordenador)"`
	CorreoOrdenador string  `orm:"column(correo_ordenador);null"`
	Estado          float64 `orm:"column(estado)"`
}
...
// Id doesn't exist
func GetOrdenadorGastoById(id int) (v *OrdenadorGasto, err error) {
	o := orm.NewOrm()
